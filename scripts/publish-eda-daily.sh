#!/usr/bin/env bash
# Generate, validate, materialize, and publish at most one EDA edition article.
set -euo pipefail
...
RUN_ROOT="$REPO/var/runs/eda/$PUBLICATION_ID-sequential"
PUBLICATION_RUN_DIR="${NEWS_ROOM_PUBLICATION_RUN_DIR:-$REPO/var/runs/publications/$PUBLICATION_ID}"
AI_DECISION_FILE="$PUBLICATION_RUN_DIR/ai-decision.json"
DECISION_FILE="$PUBLICATION_RUN_DIR/eda-decision.json"
...
PROMPT_FILE="$REPO/prompts/daily-eda-codex.md"
REPAIR_PROMPT_FILE="$REPO/prompts/repair-eda-candidate.md"
COPYEDIT_RUNNER="$REPO/scripts/run-technical-korean-copyedit.sh"
BRIEF_FILE="$REPO/prompts/eda-briefs/$PUBLICATION_ID.md"
...
if ! git cat-file -e "HEAD:content/$PUBLICATION_ID/article.md" 2>/dev/null; then
  echo "EDA publication requires the current-affairs publication for $PUBLICATION_ID" >&2
  exit 2
fi
"$REPO/scripts/verify-publication.sh" current-affairs "$PUBLICATION_ID" >/dev/null

AI_STAGE_COMPLETE=false
if git cat-file -e "HEAD:content/ai/$PUBLICATION_ID/article.md" 2>/dev/null; then
  "$REPO/scripts/verify-publication.sh" ai "$PUBLICATION_ID" >/dev/null
  AI_STAGE_COMPLETE=true
elif [[ -f "$AI_DECISION_FILE" ]] && python3 - "$AI_DECISION_FILE" "$PUBLICATION_ID" <<'PY'
import json, sys
...
if [[ "$AI_STAGE_COMPLETE" != true ]]; then
  echo "EDA publication requires a completed AI publish or no-publish stage" >&2
  exit 2
fi
...
PY
then
  echo "EDA no-publish decision already exists for $PUBLICATION_ID"
  exit 0
fi

if git cat-file -e "HEAD:content/eda/$PUBLICATION_ID/article.md" 2>/dev/null; then
  echo "EDA publication already exists for $PUBLICATION_ID"
  exit 0
fi
if [[ ! -f "$PROMPT_FILE" || ! -f "$REPAIR_PROMPT_FILE" || ! -x "$COPYEDIT_RUNNER" ]]; then
  echo "EDA publication, repair, or Korean copy-edit entrypoint is missing" >&2
  exit 2
fi
if [[ ! "$MAX_VALIDATION_ATTEMPTS" =~ ^[12]$ ]]; then
  echo "NEWS_ROOM_EDA_MAX_VALIDATION_ATTEMPTS must be 1 or 2" >&2
  exit 2
fi

mkdir -p "$STAGED_DIR" "$PUBLICATION_RUN_DIR"
cat > "$REQUEST" <<EOF
{
  "schema_version": 1,
...
if [[ $SESSION_EXIT -ne 0 ]]; then
  echo "EDA editorial turn failed with exit code $SESSION_EXIT" >&2
  exit "$SESSION_EXIT"
fi

if [[ -f "$NO_PUBLISH" ]]; then
  if [[ -f "$ARTICLE" || -f "$EVIDENCE" ]]; then
    echo "EDA turn produced both no-publish and publication artifacts" >&2
    exit 2
  fi
...
PY
  if [[ -n "$(git status --porcelain=v1 --untracked-files=all)" ]]; then
    echo "EDA no-publish turn modified tracked publication files" >&2
    git status --short >&2
    exit 2
  fi
  cp "$NO_PUBLISH" "$DECISION_FILE.tmp"
  mv "$DECISION_FILE.tmp" "$DECISION_FILE"
  echo "EDA edition: no-publish for $PUBLICATION_ID"
  exit 0
fi

if [[ ! -f "$ARTICLE" || ! -f "$EVIDENCE" ]]; then
  echo "EDA turn produced neither a complete candidate nor no-publish decision" >&2
  exit 2
fi

"$COPYEDIT_RUNNER" eda "$REQUEST" "$ARTICLE" "$EVIDENCE" "$RUN_DIR" \
...
run_candidate_validation() {
  ...
  : > "$log"
  if ! python3 scripts/publish-eda-candidate.py \
    --article "$ARTICLE" \
    --evidence "$EVIDENCE" \
...
repair_candidate() {
  ...
  if [[ $repair_exit -ne 0 ]]; then
    echo "EDA candidate repair turn failed with exit code $repair_exit" >&2
    return "$repair_exit"
  fi
}
...
while ! run_candidate_validation "$VALIDATION_ATTEMPT"; do
  if (( VALIDATION_ATTEMPT >= MAX_VALIDATION_ATTEMPTS )); then
    echo "EDA candidate remained invalid after $VALIDATION_ATTEMPT validation attempt(s)" >&2
    exit 2
  fi
...
if [[ -n "$(git status --porcelain=v1 --untracked-files=all)" ]]; then
  echo "EDA candidate turn modified files outside the ignored run directory" >&2
  git status --short >&2
  exit 2
fi
...
python3 scripts/publish-eda-candidate.py \
  --article "$ARTICLE" \
  --evidence "$EVIDENCE" \
  --publication-id "$PUBLICATION_ID" \
...
"$REPO/scripts/finalize-publication.sh" eda "$PUBLICATION_ID"
"$REPO/scripts/verify-publication.sh" eda "$PUBLICATION_ID"
