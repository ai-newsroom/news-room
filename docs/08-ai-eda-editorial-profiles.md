...
`editions/ai/sources/primary-sources.json`은 강제 체크리스트가 아니라 발견을 시작하는
공개 seed 목록이다. 신호 수집자는 특정 지역·언어·회사 인지도나 기존 기사 관성에
치우치지 않고 등록 밖의 공식 원문도 탐색한다. Qwen·DeepSeek처럼 공식 blog보다 model
registry나 GitHub에 artifact가 먼저 나타날 수 있는 공급자는 발표 surface와 artifact
surface를 함께 본다. 새 공급자나 공개 경로가 확인되면 다음 실행이 참고할 seed로
레지스트리를 갱신할 수 있지만, 한 URL의 장애나 누락만으로 발행을 실패시키지는 않는다.

발견한 실질적 모델·API·SDK·runtime 발표는 선택한 것만이 아니라 탈락한 것까지 후보
원장에 남긴다. 원장에는 발표 시각, 공식 source URL, 다섯 선정 점수, 선택 여부와 이유가
들어간다. 같은 날 공개된 주요 모델 발표가 있으면 infrastructure·tool release와 반드시
같은 표에서 비교한 뒤 선택한다. 이 기록은 공급자별 기사 할당량을 맞추기 위한 것이
아니라, 편집국이 보지 못한 후보를 "편집 판단"으로 오인하지 않기 위한 회고 자료다.
...
AI판의 기사 prompt와 문체 validator는 이 계약 ID를 명시적으로 참조합니다. 주 독자는
AI 모델·API·SDK·오픈소스를 제품과 개발 환경에 적용할지 판단하는 SW 엔지니어입니다.
AI 연구 경험이나 해당 논문의 사전 지식은 요구하지 않습니다. 동료 엔지니어가 설계 검토에서
근거와 선택지를 설명하듯 쓰되, 기술 깊이와 검증 기준을 낮춰 쉬워 보이게 만들지는 않습니다.

1. 첫 문단은 **무엇이 바뀌었는지**, **어떤 개발 작업에 영향을 주는지**, **왜 지금
   확인할 가치가 있는지**를 바로 설명합니다. 회사 소개, 행사 분위기, 추상적인 전망으로
   시작하지 않습니다.
2. 세 줄 요약 다음에는 SW 엔지니어가 빠르게 판단할 수 있도록 `지금 확인할 수 있는 것`,
   `도입 전에 확인할 것`, `아직 결론 내릴 수 없는 것`을 주제에 맞게 짧게 정리합니다.
3. 기사에는 `이 공개의 의의와 편집 판단` 절을 둡니다. 먼저 의의를 뒷받침하는 확인된
   사실을 밝히고, 그 사실이 개발자의 선택이나 기술 흐름에 어떤 의미가 있는지
   `편집 판단:`으로 구분해 씁니다. 의견은 근거 등급을 올리지 않으며, “업계를 바꾼다”처럼
   검증할 수 없는 전망 대신 달라지는 기술 판단을 구체적으로 설명합니다.
4. 본문은 원칙적으로 `변경점 → 입력에서 출력까지의 동작 → API·runtime·배포에 미치는
   영향 → 직접 검증할 방법 → 성능 수치의 조건과 한계` 순서로 전개합니다. 구현과 무관한
   연구 배경은 결론을 이해하는 데 필요한 만큼만 둡니다.
5. 제목, 설명형 소제목, 표의 고정 라벨, 원문 직접 인용, 고유명사를 제외한 서술 본문은
   `합니다체`를 일관되게 사용합니다. 지나치게 격식을 차린 보고서체, 강의하듯 훈계하는
   표현, 반말과 유행어는 쓰지 않습니다.
6. API, SDK, runtime, latency, throughput, commit처럼 SW 엔지니어가 일상적으로 쓰는
   용어는 억지로 번역하지 않습니다. AI 분야에만 익숙한 용어와 약어는 처음 등장할 때
   한국어 설명, 원어 또는 약어, 이 글에서 맡는 역할을 함께 밝힙니다.
7. 설치·호출·재현 절차를 제시할 때는 확인한 version·commit, 실행 환경, 필수 조건을
   같이 씁니다. 직접 실행하지 않은 명령이나 코드는 실행 예제로 단정하지 않고, 문서에서
   확인한 절차와 편집국이 실제 재현한 결과를 구분합니다.
8. 성능은 개발자가 비교 가능한 단위로 설명합니다. latency와 throughput, 평균과
   tail latency, 품질과 비용, 단일 요청과 concurrency를 섞지 않으며, 비교 조건이 다르면
   순위를 만들지 않습니다.
9. 한 문단에는 한 논점만 두고 대체로 2~4문장으로 씁니다. 소제목은 “구조”, “결과” 같은
   명사 한 단어보다 독자가 그 절에서 알게 될 내용을 설명하는 문장형으로 씁니다.
10. 친근함은 짧고 직접적인 설명에서 만듭니다. 독자가 모르는 것을 탓하는 “당연히”,
   “누구나 알듯”, “간단히 말해”를 피하고, 홍보 문구·과장·밈·억지 비유를 쓰지 않습니다.
   비유가 꼭 필요하면 하나만 쓰고 대응 관계와 비유가 성립하지 않는 범위를 바로 밝힙니다.
11. 상세 claim 원장과 전체 출처는 본문 뒤에 두되, 수치의 조건·재현 부재·이해상충은
    해당 주장 바로 옆에서도 밝힙니다. 문체를 고칠 때 claim의 의미, 수치, 단위, URL,
    version·commit, license, 근거 등급, 재현성, benchmark 조건, 한계, 이해상충을 바꾸지
    않습니다. 기사 hash가 달라지면 evidence와 release metadata를 다시 검증합니다.
