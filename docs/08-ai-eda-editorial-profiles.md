...
문체는 두 층으로 적용한다. `docs/16-korean-writing-style.md`가 모든 판에 공통인 자연스러운
한국어 문장을 정하고, 이 문서는 그 위에서 AI·EDA 기술 기사의 독자, 설명 깊이, 근거와
출고 기준을 정한다. 시사판의 문장 리듬이나 논평 구조를 기술판의 공통 문체로 삼지 않는다.
...
### 2.1 기사 문장과 검증 기록의 역할 분리

기술판의 근거 규칙은 기사 품질을 지키는 편집 장치이지 독자가 읽어야 할 주인공이 아니다.
제목과 도입부는 확인된 사건, 변화의 의미, 영향을 받는 독자를 먼저 보여 준다. 근거 등급,
재현성 상태, 비교 조건, 이해상충은 본문에서 필요한 위치와 근거 원장에 정확히 남기되,
같은 주의 문구를 제목·요약·판단 상자·여러 본문 절에서 반복하지 않는다.

제목은 원칙적으로 회사명보다 기술·제품·모델·논문·프로젝트 이름을 먼저 놓고, 확인된
변화와 핵심 기술 의미를 짧게 이어 쓴다. `기술명 + 공개·출시·변경 - 핵심 의미`는 의미의
우선순위이지 고정된 구두점이나 문형이 아니다. 회사·기관 자체가 사건의 주인공이거나
대상을 식별하는 데 꼭 필요할 때만 주체명을 앞세운다. 한계가 사건 자체의 핵심이 아니라면
`하지만`, `조건부`, `봐야 한다`를 붙여 검증 판정문처럼 만들지 않는다. 한계는 부제 역할의
summary와 해당 근거를 설명하는 절에서 다룬다. 본문은 구체적인 사용 장면을 먼저 제시하고
필요한 기술 개념을 뒤이어 설명해, 비전문 독자는 앞부분에서 핵심을 이해하고 엔지니어는
뒤로 갈수록 구현·조건·근거를 깊게 읽을 수 있게 한다.
...
## 이번 뉴스가 바꾼 것                 # 예시: 뉴스 설명 층
## 장기 작업 환경이 모델을 훈련하는 방식 # 예시: 기술 이해 층
## 기존 학습 방식과 비교하면            # 필요할 때만 사용하는 선택 절
## 기술적 의미와 검증 과제               # 예시: 의미와 검증 층
## 이해상충과 취재 조건
## 근거 원장
## 출처
```

본문 소제목은 예시이며 기사마다 바꾼다. 모든 기술판 기사는 뉴스 자체의 변화, 쉽고
심층적인 기술 이해, 기술적 의미와 검증 과제라는 세 의미 층을 전달한다. 작동 원리,
이전 기술과의 차이, 실제로 가능해진 작업, 구현 예, benchmark는 주제에 필요한 것만
선택한다. 세 의미 층을 고정 목차나 고정 순서로 해석하지 않는다.

`evidence_ceiling`은 기사 안에서 가장 높은 등급이 아니라 **제목과 summary의 중심 결론을
받치는 등급**이다. `이해상충과 취재 조건`, `근거 원장`, `출처`는 본문 뒤 검증 부록으로
필수다. 재현성과 한계는 관련 주장이나 의미와 검증 층에서 한 번 명확히 설명한다.
...
않는다. AI판은 검증된 날짜별 미발행 결정을 첫 화면의 짧은 발행 상태로 표시할 수 있지만,
기사 route·RSS 항목·소셜 카드로 만들지 않는다. 실행 실패는 별도 run 상태로 기록하고,
미발행 결정인 것처럼 포장하지 않는다.
...
AI 연구 경험이나 해당 논문의 사전 지식은 요구하지 않습니다. 기술을 처음 접하는
엔지니어도 앞에서부터 따라올 수 있는 뉴스 기사로 쓰되, 이미 아는 독자에게 필요한 기술
깊이와 검증 기준도 유지합니다. 모든 문장은 먼저 `korean-writing-baseline-v1`을 따릅니다.

1. 첫 문단은 **무엇이 바뀌었는지**, **어떤 개발 작업에 영향을 주는지**, **왜 지금
   확인할 가치가 있는지**를 바로 설명합니다. 회사 소개, 행사 분위기, 추상적인 전망으로
   시작하지 않습니다.
2. 본문은 뉴스 설명, 쉽고 심층적인 기술 이해, 기술적 의미와 검증 과제라는 세 의미 층을
   전달합니다. 이 셋은 고정 소제목이나 고정 순서가 아니며 한 절이 두 층을 연결하거나
   기술 이해를 여러 절로 나눌 수 있습니다.
3. 기술 이해에는 architecture·training·data·post-training, 입력에서 출력까지의 동작,
   이전 기술과의 차이, 실제로 가능해진 작업, API·runtime·agent 흐름, benchmark 설계 중
   주제에 필요한 것만 사용합니다. 모든 기사를 같은 비교표나 사용 예에 맞추지 않습니다.
4. 기술적 의미는 앞에서 설명한 원리와 변화에서 도출하고, 적용 범위·trade-off·한계·
   미검증 조건을 같은 맥락에서 정리합니다. `세 줄 요약`, `SW 엔지니어를 위한 판단`,
   `확인된 것과 확인되지 않은 것`, `편집 판단`을 고정 상자로 두지 않습니다.
...
9. 한 문단에는 한 논점만 두고 대체로 2~4문장으로 씁니다. 소제목은 “구조”, “결과” 같은
   명사 한 단어나 추상적인 판정문보다 독자가 그 절에서 알게 될 구체적인 내용을
   자연스러운 한국어로 씁니다.
...
11. 이해상충과 취재 조건, 상세 claim 원장, 전체 출처는 본문 뒤 검증 부록에 둡니다.
    수치의 조건과 재현 부재는 해당 주장 바로 옆에서도 한 번 밝히되 여러 절에서 같은
    경고를 반복하지 않습니다. 문체를 고칠 때 claim의 의미, 수치, 단위, URL,
    version·commit, license, 근거 등급, 재현성, benchmark 조건, 한계, 이해상충을 바꾸지
    않습니다. 기사 hash가 달라지면 evidence와 release metadata를 다시 검증합니다.
12. 초고 뒤에는 `korean-writing-baseline-v1`만 적용하는 한국어 편집 턴을 매번 실행합니다.
    이 편집은 새 취재나 기술 판단을 하지 않고, 제목·요약·소제목·설명 본문의 번역투,
    추상적인 조어, 명사 과잉과 문장 호흡만 고칩니다. 수치·URL·코드 식별자·근거 부록과
    evidence가 달라지면 편집을 폐기하고 출고를 중단합니다.
