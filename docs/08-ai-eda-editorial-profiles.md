...
제목은 원칙적으로 `주체 + 확인된 변화`를 담은 뉴스형 문장으로 쓴다. 한계가 사건 자체의
핵심이 아니라면 `하지만`, `조건부`, `봐야 한다`를 붙여 검증 판정문처럼 만들지 않는다.
한계는 부제 역할의 summary와 해당 근거를 설명하는 절에서 다룬다. 본문은 구체적인 사용
장면을 먼저 제시하고 필요한 기술 개념을 뒤이어 설명해, 비전문 독자는 앞부분에서 핵심을
이해하고 엔지니어는 뒤로 갈수록 구현·조건·근거를 깊게 읽을 수 있게 한다.
