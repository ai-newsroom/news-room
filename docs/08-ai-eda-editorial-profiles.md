...
#### AI 기술 블로그 문체 계약 (`ai-technical-blog-v1`)

AI판의 기사 prompt와 문체 validator는 이 절의 계약 ID를 명시적으로 참조한다. 목표는
검증 보고서를 가볍게 만드는 것이 아니라, 개발자와 연구자가 근거를 놓치지 않고 끝까지
읽을 수 있는 한국어 기술 블로그로 편집하는 것이다.

1. 첫 문단은 **무엇이 바뀌었는지**와 **누구의 어떤 판단에 중요한지**를 바로 말한다.
   회사 소개, 행사 분위기, 추상적인 전망으로 시작하지 않는다.
2. 본문은 원칙적으로 `무엇이 바뀌었나 → 구조가 어떻게 동작하나 → 공개 범위와 직접
   써볼 때의 판단 → benchmark를 어디까지 믿을 수 있나`의 독자 흐름을 따른다. 주제에
   맞지 않는 절은 이름을 바꿀 수 있지만 네 질문에 대한 답은 남긴다.
3. 한 문단에는 한 논점만 두고 대체로 2~4문장으로 쓴다. 소제목은 “구조”, “결과” 같은
   명사 한 단어보다 독자가 그 절에서 알게 될 내용을 설명하는 문장형으로 쓴다.
4. 시스템 기사는 구성 요소의 나열보다 입력이 어떤 경로를 거쳐 어떤 출력이 되는지,
   실제 도입 판단에서 무엇을 측정해야 하는지를 먼저 설명한다. 전문 용어와 약어는 처음
   나올 때 한국어로 풀고 이후에만 줄여 쓴다.
5. 코드·표·목록은 실행 순서, 조건 비교, claim 추적처럼 산문보다 이해를 실제로 돕는
   경우에만 쓴다. 장식용 코드 조각, 표로 옮긴 문장, 긴 체크리스트는 만들지 않는다.
6. 친근함은 짧고 직접적인 설명에서 만든다. 홍보 문구, 과장, 밈, 억지 비유, 시사판의
   좌우 persona·찬반 토론·닫는 질문을 가져오지 않는다.
7. 상세 claim 원장과 전체 출처는 본문 뒤에 두되, 성능 수치의 조건·재현 부재·이해상충은
   해당 주장 바로 옆에서도 밝힌다. 독자가 한계를 보려고 글 끝까지 내려가게 만들지 않는다.
8. 문체를 고칠 때 claim의 의미, 수치, 단위, version·commit, 근거 등급, 재현성, 한계,
   이해상충을 바꾸지 않는다. 공개 기사의 byte hash가 달라지면 evidence hash는 재검증하고
   사람 승인 release metadata의 article hash만 실제 파일에 맞춰 갱신한다.
