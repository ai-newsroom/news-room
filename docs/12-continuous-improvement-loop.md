...
`YYYY-MM-DD--<slug>` 식별자를 사용한다. 공개 검증에 성공하면 09:30 routine과 같은
`origin/main` 회고 계약을 기존 컨덕터 세션에 직접 전달한다. 발행기가 확인한 URL·제목·발행
ID를 검증 입력으로 함께 보내 중복 network 요청을 피한다. 회고 실패가 이미 공개된 기사를
되돌리지는 않는다.
